...
class ControlThread:
    """Threaded handler for drone control commands."""
    def __init__(self):
        self.control_thread: Optional[threading.Thread] = None
        self._lock = threading.Lock()
        self.running = False

    def handle_command(self, drone, command, distance=30):
    ...
    def start(self, drone, command, distance=30):
...
# Initialize VideoStreamer and ControlThread
...
def handle_drone_command(data):
    ...
    try:
        control_thread.start(drone, command, distance)
        emit('message', {'data': f"Executing {command} command"})
    except Exception as e:
        logger.error(f"Error executing command {command}: {str(e)}")
